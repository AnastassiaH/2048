...
const move = (start, endX, endY, step, stepX, stepY)=>{
    for(let x = 0; x < endY; x += step){
        const line = [];
        for(let i = 0 + x; i < fieldCells.length; i += stepY)if (line.length < 4) line.push(fieldCells[i]);
        ...
        for(let i = start; i !== endX; i += stepX)if (collapseLine(i, stepX)) moveLine(i, stepX);
    }
};
